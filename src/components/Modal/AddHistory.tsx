...
const serverUrl = 'socket.ordersdropship.com';
...
const AddHistory = ({
  ...
  const mutationAdd = useMutation((body: any) => {
    return UpdateHistory(userId._id, body)
  })
  ...
  const handleAdd = () => {
    const newData = {
      info: valueView,
      status: 'done',
      activate: `đã xác nhận cộng tiền ${userId.nameUser} với mã ${userId.codeOder}`,
  ...
  const handleMinus = () => {
    const newData = {
      ...
      userId: userId._id,
      status: 'deny',
      activate: `đã huỷ trừ tiền ${userId.username} với mã ${userId.codeOder}`,
